...
def play_episode(env, agents):
    ...
    for a in agents:
        a.step(ts)  # terminal bookkeeping
    return ts.rewards


def eval_vs_random(game, agents, games_per_seat, seed_base):
        ...
        for g in range(games_per_seat):
            ...
            with agents[seat].temp_mode_as(nfsp.MODE.AVERAGE_POLICY):
                while not ts.last():
                    pid = ts.observations["current_player"]
                    if pid == seat:
                        act = agents[seat].step(ts, is_evaluation=True).action
                    else:
...
def main(argv=None):
    ...
    for ep in range(start + 1, args.episodes + 1):
        ...
        if ep % args.eval_every == 0 or ep == args.episodes:
            ev = eval_vs_random(game, agents, args.eval_games, args.eval_seed)
